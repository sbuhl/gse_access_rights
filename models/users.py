...
class Users(models.Model):
    ...
    sbu_allowed_warehouses_ids = fields.Many2many(
        "stock.warehouse", string="Allowed Warehouse"
    )
    ...
    def write(self, values):
        ...
        for user in self:
            ...
            for field in to_track:
                ...
                if new_values:
                    body.append(
                        f"- a ajouté l'accès à [{len(new_values)} {new_values._description}] : {new_values.mapped('display_name')}"
                    )
                if removed_values:
                    body.append(
                        f"- a retiré l'accès à [{len(removed_values)} {removed_values._description}] : {removed_values.mapped('display_name')}"
                    )
            if body:
                body.insert(0, autor)
